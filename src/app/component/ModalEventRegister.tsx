...
export default function ModalEventRegister({
  ...
  const executeBatchUnregistration = async (ids: string[]) => {
    ...
    if (okCount > 0) {
      toast.success(`Đã hủy ${okCount} sự kiện.`, { id: loadId });
  ...
  const displayedAvailableEvents = useMemo(
    () =>
      filterEvents(availableEvents.filter((event) => !isRegistered(event.id))),
    [availableEvents, filterEvents, isRegistered]
  ); // Lọc bỏ sự kiện đã đăng ký khỏi tab available
  const displayedRegisteredEvents = useMemo(
    () => filterEvents(registeredEvents),
    [registeredEvents, filterEvents]
  );
  ...
  const handleSelectAllForUnregister = (
    ...
    if (isChecked) {
      setSelectedToUnregister(
        new Set(displayedRegisteredEvents.map((ev) => ev.id))
      );
    } else {
      setSelectedToUnregister(new Set());
    }
