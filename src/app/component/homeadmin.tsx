...
export default function HomeAdmin() {
    ...
    async (token: string | null) => {
      ...
      try {
        ...
        if (data.code === 1000 && Array.isArray(data.result)) {
          const formattedEvents: EventDisplayInfo[] = data.result.map(
            (apiEvent: any) => ({
              id: apiEvent.id,
              title: apiEvent.name || "Chưa có tiêu đề",
              date: apiEvent.time || new Date().toISOString(),
        ...
        if (
          ...
          // Hook refreshToken đã xử lý chuyển hướng
          // handleLogout(); // Có thể gọi logout nếu muốn đảm bảo dọn dẹp
        }
      } finally {
        setIsLoadingEvents(false);
      }
    ...
    const loadAdminData = async () => {
      ...
      if (currentAuthToken) {
        try {
          ...
          if (!userRes.ok)
            throw new Error(
              `Workspace user info failed: ${userRes.statusText}`
            );
          ...
          if (userData.code === 1000 && userData.result?.id) {
            ...
            if (isMounted) {
              if (!fetchedUser.roles?.some((r) => r.name === "ADMIN")) {
                ...
                setUser(fetchedUser);
                fetchAdminHomeEvents(localStorage.getItem("authToken"));
              }
            }
          } else {
  ...
  }, [isInitialized, authToken, fetchAdminHomeEvents, router, refreshToken]);
  ...
  if (!user || !user.roles?.some((r) => r.name === "ADMIN")) {
    return (
      <div className="min-h-screen flex justify-center items-center bg-gray-100 text-red-500">
        Truy cập bị từ chối hoặc phiên đăng nhập không hợp lệ.
      </div>
    );
  }
