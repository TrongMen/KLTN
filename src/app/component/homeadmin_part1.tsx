        ...
        {selectedEvent ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
            {upEvents.length > 0 ? (
              ...
              <p className="text-gray-500 text-center col-span-2">🚀 Không có sự kiện nào sắp diễn ra.</p>
            )}
          </div>
        )}
      </div>
      {showContactModal && <ContactModal onClose={() => setShowContactModal(false)} />}
      {showModalMember && <ModalMember onClose={() => setShowModalMember(false)} userRole={user?.roles?.[0] || "admin"} currentUserEmail={user?.email || ""} />}
      {showModalChat && (
        <ModalChat onClose={() => setShowModalChat(false)} onSelectConversation={handleSelectConversation} />
      )}
