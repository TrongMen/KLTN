...
type ActiveTab = 'home' | 'registeredEvents' | 'members' | 'chatList';
...
export default function HomeGuest() {
    ...
    try {
      ...
      if (d.code === 1000 && Array.isArray(d.result)) {
        const fmt: EventDisplayInfo[] = d.result.map((e: any) => ({
          ...
          time: e.time, status: e.status, purpose: e.purpose,
        }));
        setAllEvents(fmt);
      } else throw new Error(d.message || "Lỗi data");
    } catch (e: any) {
  ...
  const handleRegister = (event: EventDisplayInfo) => {
    if (!user || !user.id) { toast.error("Đăng nhập để đăng ký."); router.push("/login"); return; }
  ...
  const tabs = [
      ...
      { id: 'registeredEvents', label: '📋 Sự kiện tham gia', requiresAuth: true },
      ...
      <nav className="bg-gray-900 text-white px-4 py-4 shadow-md mb-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 sm:gap-6 text-sm sm:text-base">
              ...
              {!isLoadingUser && !user && (<Link href="/login"><span className="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded cursor-pointer">Đăng nhập</span></Link>)}
          </div>
        </div>
      </nav>
          ...
          <>
            {activeTab === "home" && (
              <HomeTabContent
                ...
                sortOption={sortOption} setSortOption={setSortOption} timeFilterOption={timeFilterOption} setTimeFilterOption={setTimeFilterOption}
              />
            )}
            {activeTab === "registeredEvents" && user && (
              <RegisteredEventsTabContent
                currentUserId={user.id} isLoadingUserId={isLoadingUser}
                registeredEventIds={registeredEventIds} createdEventIds={createdEventIds}
                onRegistrationChange={handleRegistrationChange}
              />
            )}
             {activeTab === "members" && user && (
                 <MembersTabContent
                     user={user} userRole={user.roles?.[0]?.name?.toUpperCase() || 'GUEST'}
