...
import { NewsFormData } from "./modals/CreateNewsModal";
import { NewsItem } from "./tabs/NewsFeedSection";
...
type ActiveTab = | "home" | "createEvent" | "myEvents" | "attendees" | "registeredEvents" | "members" | "chatList";
...
export default function UserHome() {
  ...
  const fetchNews = useCallback(async () => {
    ...
    try {
      ...
      const url = `http://localhost:8080/identity/api/news/status?status=PENDING`; // Endpoint để lấy news
      ...
      if (d.code === 1000 && Array.isArray(d.result)) {
        const fmt: NewsItem[] = d.result.map((item: any) => ({
          ...
          date: item.createdAt || item.publishDate || "",
  ...
  const fetchRegisteredEventIds = useCallback(async (userId: string, token: string | null) => {
    if (!userId || !token) { setIsLoadingRegisteredIds(false); setRegisteredEventIds(new Set()); return; }
    ...
    try {
      ...
      if (data.code === 1000 && Array.isArray(data.result)) setRegisteredEventIds(new Set(data.result.map((event: any) => event.id)));
      else setRegisteredEventIds(new Set());
    } catch (err: any) {
      console.error("Lỗi tải ID sự kiện đã đăng ký:", err);
      setRegisteredEventIds(new Set());
      if (err.message?.includes("Unauthorized")) router.push("/login?sessionExpired=true");
    } finally {
      setIsLoadingRegisteredIds(false);
    }
  }, [refreshToken, router]);

  const fetchUserCreatedEvents = useCallback(async (userId: string, token: string | null) => {
    if (!userId || !token) { setIsLoadingCreatedEventIds(false); setCreatedEventIds(new Set()); return; }
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP error! status: ${res.status}`);
      const data = await res.json();
      if (data.code === 1000 && Array.isArray(data.result)) setCreatedEventIds(new Set(data.result.map((event: any) => event.id)));
  ...
  useEffect(() => {
    ...
    let isMounted = true;
    setIsLoadingUser(true); setIsLoadingEvents(true); setIsLoadingRegisteredIds(true); setIsLoadingCreatedEventIds(true); setIsLoadingNews(true);
    ...
    const loadInitialData = async () => {
      ...
      if (currentAuthToken) {
        try {
          ...
          if (userRes.status === 401 || userRes.status === 403) {
            const newToken = await refreshToken();
            if (newToken && isMounted) userRes = await fetch(userInfoUrl, { headers: { Authorization: `Bearer ${newToken}` } });
            else if (isMounted) throw new Error("Unauthorized or Refresh Failed");
          }
          if (!userRes.ok) throw new Error(`Workspace user info failed: ${userRes.status}`);
          ...
          if (isMounted) setUser(null); userIdForFetches = null;
        } finally { if (isMounted) setIsLoadingUser(false); }
      } else {
        if (isMounted) { setUser(null); setIsLoadingUser(false); setIsLoadingRegisteredIds(false); setIsLoadingCreatedEventIds(false); }
      }
      ...
      if (userIdForFetches && isMounted) {
        ...
        await Promise.all([fetchRegisteredEventIds(userIdForFetches, tokenForSubFetches), fetchUserCreatedEvents(userIdForFetches, tokenForSubFetches)]);
      } else if (isMounted) { setIsLoadingRegisteredIds(false); setIsLoadingCreatedEventIds(false); }
    };
    loadInitialData();
    return () => { isMounted = false; };
  }, [isInitialized, fetchAllEvents, fetchRegisteredEventIds, fetchUserCreatedEvents, fetchNews, refreshToken]);
  ...
  const executeRegistration = async (event: EventDisplayInfo) => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        const newToken = await refreshToken();
        if (newToken) { token = newToken; res = await fetch(url, { method: "POST", headers: { Authorization: `Bearer ${token}` } }); }
      ...
      const data = await res.json();
      if (data.code === 1000) { toast.success(`Đã đăng ký "${event.title}"!`); setRegisteredEventIds((prev) => new Set(prev).add(event.id)); }
  ...
  const handleRegister = (event: EventDisplayInfo) => {
    if (!user?.id) { toast.error("Đăng nhập để đăng ký."); router.push("/login"); return; }
    ...
    if (!isEventUpcoming) { toast.error("Sự kiện này đã diễn ra."); return; }
    setConfirmationState({
      isOpen: true, title: "Xác nhận đăng ký", message: (<>Đăng ký sự kiện <br /> <strong className="text-indigo-600">"{event.title}"</strong>?</>),
      onConfirm: () => { setConfirmationState({ isOpen: false, title: "", message: "", onConfirm: null }); executeRegistration(event); },
  ...
  const handleRegistrationChange = useCallback((eventId: string, registered: boolean) => {
    setRegisteredEventIds((prevIds) => {
      ...
      if (registered) newIds.add(eventId); else newIds.delete(eventId);
      return newIds;
    });
  }, []);
  ...
  const handleLogout = async () => {
    try {
      ...
      if (t) await fetch("http://localhost:8080/identity/auth/logout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token: t }), });
    } catch (e) { console.error("Lỗi gọi API logout:", e); }
    finally {
      localStorage.removeItem("authToken"); localStorage.removeItem("refreshToken"); localStorage.removeItem("authenticated"); localStorage.removeItem("role"); localStorage.removeItem("user");
      setUser(null); setRegisteredEventIds(new Set()); setCreatedEventIds(new Set()); setActiveTab("home"); router.push("/login");
    }
  };
  ...
  const getTabButtonClasses = (tabName: ActiveTab): string => {
    ...
    switch (tabName) {
      ...
      case "createEvent": specificBg = activeTab === tabName ? "bg-cyan-600" : "bg-cyan-100"; specificText = activeTab === tabName ? "" : "text-cyan-800"; specificHoverBg = activeTab === tabName ? "hover:bg-cyan-700" : "hover:bg-cyan-200"; break;
      case "myEvents": specificBg = activeTab === tabName ? "bg-blue-600" : "bg-blue-100"; specificText = activeTab === tabName ? "" : "text-blue-800"; specificHoverBg = activeTab === tabName ? "hover:bg-blue-700" : "hover:bg-blue-200"; break;
      case "attendees": specificBg = activeTab === tabName ? "bg-teal-600" : "bg-teal-100"; specificText = activeTab === tabName ? "" : "text-teal-800"; specificHoverBg = activeTab === tabName ? "hover:bg-teal-700" : "hover:bg-teal-200"; break;
      case "registeredEvents": specificBg = activeTab === tabName ? "bg-green-600" : "bg-green-100"; specificText = activeTab === tabName ? "" : "text-green-800"; specificHoverBg = activeTab === tabName ? "hover:bg-green-700" : "hover:bg-green-200"; break;
  ...
  const getActiveIndicatorColor = (tabName: ActiveTab): string => {
    switch (tabName) {
      case "home": return "border-t-indigo-600"; case "createEvent": return "border-t-cyan-600"; case "myEvents": return "border-t-blue-600"; case "attendees": return "border-t-teal-600"; case "registeredEvents": return "border-t-green-600"; case "members": return "border-t-pink-600"; case "chatList": return "border-t-purple-600"; default: return "border-t-gray-400";
    }
  };
  const tabs = [
    { id: "home", label: "🎉 Trang chủ", requiresAuth: false }, { id: "createEvent", label: "➕ Tạo sự kiện", requiresAuth: true }, { id: "myEvents", label: "🛠 Sự kiện của tôi", requiresAuth: true }, { id: "attendees", label: "✅ Người tham gia", requiresAuth: true }, { id: "registeredEvents", label: "📋 Sự kiện đã đăng ký", requiresAuth: true }, { id: "members", label: "👥 Thành viên CLB", requiresAuth: true }, { id: "chatList", label: "💬 Danh sách chat", requiresAuth: true },
  ];
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6">
      ...
      <nav className="bg-gray-900 text-white px-4 py-4 shadow-md mb-6">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 sm:gap-6 text-sm sm:text-base">
            ...
            {isInitialized && !isLoadingUser && (<UserMenu user={user} onLogout={handleLogout} />)}
            {(!isInitialized || isLoadingUser) && (<span className="text-gray-400">Đang tải...</span>)}
            {isInitialized && !isLoadingUser && !user && (<Link href="/login"><span className="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded cursor-pointer">Đăng nhập</span></Link>)}
          </div>
        </div>
      </nav>
      <div className="max-w-7xl mx-auto bg-white shadow-md rounded-xl p-4 mb-6 border border-gray-200">
        <div className="flex flex-wrap gap-x-3 sm:gap-x-4 gap-y-5 justify-center pb-3">
          {tabs.map((tab) => {
            const showTab = !tab.requiresAuth || (tab.requiresAuth && isInitialized && user);
            if (!showTab) return null;
            return (
              <div key={tab.id} className="relative flex flex-col items-center">
                <button onClick={() => setActiveTab(tab.id as ActiveTab)} className={getTabButtonClasses(tab.id as ActiveTab)}>{tab.label}</button>
                {activeTab === tab.id && (<div className={`absolute top-full mt-1.5 w-0 h-0 border-l-[6px] border-l-transparent border-t-[8px] ${getActiveIndicatorColor(tab.id as ActiveTab)} border-r-[6px] border-r-transparent`} style={{ left: "50%", transform: "translateX(-50%)" }}></div>)}
              </div>
            );
          })}
          {isInitialized && !user && !isLoadingUser && (<span className="text-sm text-gray-500 italic p-2 self-center">Đăng nhập để xem các mục khác</span>)}
        </div>
      </div>
      <div className="max-w-7xl mx-auto bg-white shadow-lg rounded-xl p-4 sm:p-6 min-h-[400px]">
        {isPageLoading ? (<p className="text-center text-gray-500 italic py-6">Đang tải dữ liệu...</p>) : (
          <>
            {activeTab === "home" && (
              <HomeTabContent
                allEvents={allEvents} isLoadingEvents={isLoadingEvents} errorEvents={errorEvents}
                registeredEventIds={registeredEventIds} createdEventIds={createdEventIds} user={user}
                isLoadingRegisteredIds={isLoadingRegisteredIds} isLoadingCreatedEventIds={isLoadingCreatedEventIds}
                isRegistering={isRegistering} onRegister={handleRegister} onEventClick={handleEventClick}
                selectedEvent={selectedEvent} onBackToList={handleBackToList} search={search} setSearch={setSearch}
                sortOption={sortOption} setSortOption={setSortOption} timeFilterOption={timeFilterOption}
                setTimeFilterOption={setTimeFilterOption} newsItems={newsItems} isLoadingNews={isLoadingNews}
                errorNews={errorNews} onNewsCreated={handleNewsCreated}
              />
            )}
            {user && activeTab === "createEvent" && (<CreateEventTabContent user={user} onEventCreated={() => { fetchAllEvents(); const t = localStorage.getItem("authToken"); if (user?.id && t) fetchUserCreatedEvents(user.id, t); setActiveTab("myEvents"); toast.success("Sự kiện đã được tạo và đang chờ duyệt!"); }} />)}
            {user && activeTab === "myEvents" && (<MyEventsTabContent user={user} />)}
            {user && activeTab === "attendees" && (<AttendeesTabContent user={user} />)}
            {user && activeTab === "registeredEvents" && (<RegisteredEventsTabContent currentUserId={user.id} isLoadingUserId={isLoadingUser} registeredEventIds={registeredEventIds} createdEventIds={createdEventIds} onRegistrationChange={handleRegistrationChange} />)}
            {user && activeTab === "members" && (<MembersTabContent user={user} userRole={user.roles?.[0]?.name?.toUpperCase() || "UNKNOWN"} currentUserEmail={user.email || null} />)}
            {user && activeTab === "chatList" && (<ChatTabContent currentUser={user} />)}
            {activeTab !== "home" && !user && isInitialized && (<p className="text-center text-red-500 py-6">Vui lòng đăng nhập để truy cập mục này.</p>)}
          </>
        )}
      </div>
      <ConfirmationDialog
