...
interface HomeTabContentProps {
    ...
    sortOption: string; // Can be 'az' or 'za', parent should default to 'az'
...
const HomeTabContent: React.FC<HomeTabContentProps> = ({
    ...
    const processedEvents = useMemo(() => {
        ...
        if (sortOption === "za") {
             evts.sort((a, b) => b.title.localeCompare(a.title, 'vi', { sensitivity: 'base' }));
        } else { // Default to A-Z if not Z-A
            evts.sort((a, b) => a.title.localeCompare(b.title, 'vi', { sensitivity: 'base' }));
        }
        return evts;
    }, [allEvents, search, timeFilterOption, sortOption, startDateFilter, endDateFilter]);
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto items-center">
                    <div className="flex-1 sm:flex-none">
                        ...
                        >
                            {/* <option value="date">📅 Ngày gần nhất</option> <-- Đã loại bỏ */}
                            <option value="az">🔤  A - Z</option>
                            <option value="za">🔤  Z - A</option>
                        </select>
                    </div>
                    <div className="flex-1 sm:flex-none">
                        <label htmlFor="timeFilterOptionGuest" className="sr-only">Lọc thời gian</label>
                        <select
             ...
             {timeFilterOption === 'dateRange' && (
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200 shadow-sm">
                     <div>
                         <label htmlFor="startDateFilterHome" className="block text-sm font-medium text-gray-700 mb-1">Từ ngày</label>
                         <input type="date" id="startDateFilterHome" value={startDateFilter} onChange={handleStartDateChange}
                                 max={endDateFilter || undefined}
                                 className="w-full p-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                     </div>
                     <div>
                         <label htmlFor="endDateFilterHome" className="block text-sm font-medium text-gray-700 mb-1">Đến ngày</label>
                         <input type="date" id="endDateFilterHome" value={endDateFilter} onChange={handleEndDateChange}
            ...
            {selectedEvent ? (
                 <div className="p-6 border rounded-lg shadow-lg bg-gray-50">
                    <button
                     ...
                     <h2 className="text-xl font-bold mb-3 text-gray-800">{selectedEvent.title}</h2>
                     <div className="space-y-1 text-sm text-gray-700">
                         <p><strong className="font-medium text-gray-900">📅 Ngày:</strong> {new Date(selectedEvent.date).toLocaleDateString("vi-VN")}</p>
                         <p><strong className="font-medium text-gray-900">📍 Địa điểm:</strong> {selectedEvent.location}</p>
                         {selectedEvent.time && <p><strong className="font-medium text-gray-900">🕒 Thời gian:</strong> {new Date(selectedEvent.time).toLocaleTimeString("vi-VN", { hour: '2-digit', minute: '2-digit'})}</p>}
                         {selectedEvent.purpose && <p><strong className="font-medium text-gray-900">🎯 Mục đích:</strong> {selectedEvent.purpose}</p>}
                     </div>
                     <div className="mt-4 pt-3 border-t border-gray-200">
                         <p className="text-sm font-medium text-gray-900 mb-1">📜 Mô tả:</p>
                         <p className="text-sm text-gray-700 whitespace-pre-wrap">{selectedEvent.description || "Không có mô tả chi tiết."}</p>
                    </div>
                 </div>
            ) : (
                 <div className="mt-1">
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="mt-auto">
                                                {isCreatedByUser ? (
                                                    <button className="w-full px-4 py-2 rounded-lg bg-gray-200 text-gray-600 cursor-not-allowed text-sm font-medium" disabled>
                                                        ✨ Sự kiện của bạn
                                                    </button>
                                                ) : showRegisterButton ? (
                                                    <button
                                                        onClick={(e) => { if(canClickRegister) { onRegister(event); } }}
                                                    ...
                                                    <>
                                                        {user && !isEventUpcoming && !isCreatedByUser && (
                                                            <button className="w-full px-4 py-2 rounded-lg bg-gray-300 text-gray-600 cursor-not-allowed text-sm font-medium" disabled>
                                                                Đã kết thúc
                                                            </button>
                                                        )}
                                                         {!user && isEventUpcoming && (
                                                            <button
                                                                onClick={(e) => { e.stopPropagation(); toast('Vui lòng đăng nhập để đăng ký sự kiện.', { icon: '🔒'}); }}
                                             ...
                                             <li key={event.id} className="px-4 py-3 hover:bg-gray-50 flex flex-col sm:flex-row sm:items-center sm:justify-between transition-colors duration-150 ease-in-out">
                                                 <div className="flex-1 mb-2 sm:mb-0 sm:pr-4 cursor-pointer" onClick={() => onEventClick(event)}>
                                                  ...
                                                  <div className="flex-shrink-0 w-full sm:w-auto mt-2 sm:mt-0">
                                                        {isCreatedByUser ? (
                                                             <button className="w-full sm:w-auto px-3 py-1.5 rounded-md bg-gray-200 text-gray-600 cursor-not-allowed text-xs font-medium" disabled>
                                                                 ✨ Sự kiện của bạn
                                                             </button>
                                                         ) : showRegisterButton ? (
                                                             <button
                                                             ...
                                                             <>
                                                                 {user && !isEventUpcoming && !isCreatedByUser && (
                                                                     <button className="w-full sm:w-auto px-3 py-1.5 rounded-md bg-gray-300 text-gray-600 cursor-not-allowed text-xs font-medium" disabled>
                                                                         Đã kết thúc
                                                                     </button>
                                                                 )}
                                                                  {!user && isEventUpcoming && (
