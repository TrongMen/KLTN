...
const MenuBar: React.FC<MenuBarProps> = ({ editor, disabled = false }) => {
  ...
  const MenuButton: React.FC<{
    ...
    <button
      ...
      onClick={onClick}
      disabled={isDisabled}
      className={`p-1 px-2 m-0.5 border border-gray-300 rounded hover:bg-gray-100 disabled:opacity-50 ${
        isActive ? "bg-gray-200 ring-1 ring-inset ring-gray-400" : "bg-white"
      }`}
      style={style}
      title={title}
    >
  ...
  return (
    <div className="menu-bar flex flex-wrap items-center gap-1 p-1 border-b border-gray-300 bg-gray-50 rounded-t-md">
      ...
      <MenuButton
        ...
        isDisabled={
          !editor.can().chain().focus().toggleBlockquote().run() || disabled
        }
      ...
      <MenuButton
        ...
        isDisabled={!editor.can().chain().focus().undo().run() || disabled}
        title="Undo"
      >
        ↶
      </MenuButton>
      <MenuButton
        ...
        isDisabled={!editor.can().chain().focus().redo().run() || disabled}
        title="Redo"
      >
        ↷
      </MenuButton>
